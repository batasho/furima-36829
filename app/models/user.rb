class User < ApplicationRecord
  ...
  validates :first_name, presence: true ,format: { with: /\A[ぁ-んァ-ンー-龥]+\z/,message: "全角ひらがな、カタカナ、漢字のみが使えます" }
  validates :last_name, presence: true ,format: { with: /\A[ぁ-んァ-ンー-龥]+\z/,message: "全角ひらがな、カタカナ、漢字のみが使えます" }
